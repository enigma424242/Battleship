...
class Location {
public:
    Location(int x = -1, char y = '*');// void constructor, assigns -1 to X coord, and * to Y coord 
    ...
    friend bool compare(const Location& ship, const Location& cord);
...
class Ship {
public:
    Ship(bool sunk = false); // void constructor, sets sunk=false
